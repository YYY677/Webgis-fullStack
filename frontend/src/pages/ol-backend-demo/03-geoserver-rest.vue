      ...
      <Transition name="fade">
        <div v-if="panelOpen" class="gs-panel">
          ...
          <template v-if="activeTab === 'workspace'">
            <el-table :data="wsList" stripe size="small" max-height="280" style="width: 100%"
              v-loading="loading" empty-text="暂无工作空间">
          ...
          <template v-if="activeTab === 'datastore'">
            <div class="gs-cascade">
              ...
              <el-select v-model="selectedWs" size="small" placeholder="选工作空间" style="width: 160px"
                clearable @change="onWsChange">
                <el-option v-for="w in wsList" :key="w.name" :label="w.name" :value="w.name" />
              </el-select>
            </div>
            <el-table :data="tableData" stripe size="small" max-height="220" style="width: 100%"
              v-loading="loading" empty-text="暂无数据存储">
              <el-table-column prop="name" label="名称" show-overflow-tooltip />
              <el-table-column label="操作" width="150" fixed="right">
                <template #default="{ row }">
          ...
          <template v-if="activeTab === 'featuretype'">
            <div class="gs-cascade" style="margin-bottom: 4px;">
              <span class="cascade-label">工作空间</span>
              <el-select v-model="ftWs" size="small" placeholder="选工作空间" style="width: 110px"
                @change="onFtWsChange">
                <el-option v-for="w in wsList" :key="w.name" :label="w.name" :value="w.name" />
              </el-select>
              ...
              <el-table-column label="操作" width="230" fixed="right">
                <template #default="{ row }">
                  ...
                  <el-button v-else size="small" text type="primary"
                    @click="publishFt(row.name)">发布</el-button>
                  <el-button size="small" text type="danger"
                    @click="dropFt(row.name, row.published)">删除</el-button>
          ...
          <template v-if="activeTab === 'layer'">
            <div class="gs-cascade">
              <span class="cascade-label">工作空间</span>
              <el-select v-model="selectedWs" size="small" placeholder="全部" clearable style="width: 160px" @change="doRefresh">
                <el-option v-for="w in wsList" :key="w.name" :label="w.name" :value="w.name" />
              </el-select>
            </div>
            <el-table :data="tableData" stripe size="small" max-height="240" style="width: 100%"
              v-loading="loading" empty-text="暂无已发布图层">
              ...
              <el-table-column label="操作" width="190" fixed="right">
                <template #default="{ row }">
                  ...
                  <el-button v-else size="small" text type="warning"
    ...
    <el-dialog v-model="detailVisible" :title="detailTitle" width="55%" top="5vh" destroy-on-close>
      ...
      <template v-else-if="detailType === 'featuretype' && detailData?.featureType">
        ...
        <el-table :data="ftAttributes" stripe size="small" max-height="300">
          ...
          <el-table-column prop="binding" label="类型" width="180" />
          <el-table-column prop="nillable" label="可为空" width="70">
...
import {
  getWorkspaces, getDataStores, getLayers,
  listAllFeatureTypes, getFeatureTypes, getDataStoreDetail,
  getFeatureTypeDetail, getLayerDetail,
  createWorkspace, deleteWorkspace,
  createDataStore, deleteDataStore,
  publishFeatureType, deleteFeatureType,
  dropFeatureType, uploadFeatureTypeFile,
  type WorkspaceItem, type DataStoreItem
} from "@/api/geoserver"
...
const ftCombined = computed(() => {
  ...
  ftAll.value.forEach(n => { if (!pubSet.has(n)) items.push({ name: n, published: false }) })
  return items
})
...
const createTitle = computed(() => activeTab.value === "workspace" ? "创建工作空间" : "创建数据存储")
...
const createServices = ref<string[]>(["wms", "wfs", "wmts", "wcs"])
...
const wfsStyle = new Style({
  ...
  image: new CircleStyle({
    radius: 5, fill: new Fill({ color: "#3388ff" }),
...
async function handleUpload() {
  ...
  try {
    const res = await uploadFeatureTypeFile(
      ftWs.value, ftDs.value, uploadName.value,
      uploadFormat.value, uploadFiles.value
    )
...
async function deleteWs(name: string) {
  ...
  try {
    ...
    const { value } = await ElMessageBox.prompt(
      ...
      "二次确认", { confirmButtonText: "确认删除", cancelButtonText: "取消",
...
async function publishFt(tableName: string) {
  try {
    await ElMessageBox.confirm(
      `确定要发布「${tableName}」为要素类型 + 图层吗？`, "确认发布",
...
function addLayer(layerFullName: string) {
  ...
  layerInfos.value = [...layerInfos.value, { id: `gs-${layerFullName}`, name: layerFullName, layer: wfsLayer as any }]
...
onMounted(async () => {
  try { const res = await getWorkspaces(); wsList.value = res.data ?? [] } catch { /* */ }
...
.gs-panel {
  ...
  width: 440px; box-shadow: var(--el-box-shadow-light); padding: 0 12px 12px;
}

.gs-toolbar { display: flex; gap: 6px; margin: 6px 0 8px; }

.gs-cascade {
  display: flex; align-items: center; gap: 4px; padding: 6px 8px;
  background: var(--el-fill-color-light); border-radius: 4px; margin-bottom: 6px;
  .cascade-label { font-size: 12px; color: var(--el-text-color-secondary); white-space: nowrap; }
}
