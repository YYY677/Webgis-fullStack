...
- 按需使用 `.agents/skills` 目录下的技能
- 任何时候都禁止主动删除文件，必须经过同意

# 本项目定制

**原名 v3-admin-vite**，已改造为 WebGIS 前端。以下改动 AI 必须了解：

## Mock 登录

`vite.config.ts` 中的 `mockPlugin()` 拦截 `/api/auth/login` 和 `/api/auth/me`，无需后端即可登录（任意用户名/密码）。后端启动后请求自动走 `/api` 代理到 `localhost:8080`。

## 响应码

axios 拦截器已从 `code === 0` 改为 `code === 200` 匹配后端 `Result<T>` 统一响应体。

## Vite 代理

开发时 `/api` → `localhost:8080`, `/geoserver` → `localhost:8081`。

## 解释更改

每次修改前端工程文件后向用户解释这些修改。
